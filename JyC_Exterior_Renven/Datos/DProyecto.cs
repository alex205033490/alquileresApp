...
namespace jycboliviaASP.net.Datos
{
    public class DProyecto
    {
        ...
        public bool modificarProyectoEncargadoPagoPP(int codigo, string nombre, string direccion, int codigoEncargado, int codigoResponsable, int estado, int codzona, string departamento, string nit, string facturar, string banco, int codcobradorasignado) 
        {
        ...
        {
            string consulta = "select  p.codigo, p.nombre, p.nombre2, p.direccion, DATE_FORMAT(p.fecha_Proy,'%d/%m/%Y') as Fecha, p.codEncargado, p.codigores , p.estado, p.codzona, p.departamento, p.nit_proy, p.facturar_proy, p.banco_proy, p.empresacontratoproyecto  from tb_proyecto p where p.estado =1 and p.codigo = " + codigo;
            DataSet lista = conexion.consultaMySql(consulta);
            return lista;
        }
        ...
        public DataSet existeProyecto(int codProyecto)         
         ...
         public bool insertarProyecto1(string nombre, string nombre2, string fecha_Proy, int codigoEncargado, int estado, string ciudad, int codpropietario)
         {
                 ...
                 string consulta = "insert into tb_proyecto(nombre, nombre2, fecha_Proy, codEncargado, estado, departamento,codpropietario) "+
                                   " values('"+nombre+"','"+nombre2+"', '"+fecha_Proy+"', "+codencargadoAux+", "+estado+", '"+ciudad+"',"+codPropietarioAux+"); ";
                 return conexion.ejecutarMySql(consulta);
         ...
         public bool modificarProyecto1(int codProyecto,string nombre,  string fecha_Proy, int estado, string ciudad)
         {
        ...
        ///
         ...
         internal DataSet getencargadoObraProyecto(int codigoEdificio)
         {
             string consulta = "SELECT " +
                                " eproy.codigo, " +
                                " eproy.nombre, " +
                                ...
                                " eproy.observacion  " +
                                " FROM  " +
                                " tb_proyecto pp , tb_encargado_obraproyectos eproy " +
                                " where " +
                                " pp.codencargadoproyecto = eproy.codigo and " +
                                " pp.codigo = " + codigoEdificio;
             return conexion.consultaMySql(consulta);
         }

         internal bool actulizarEncargadoObraProyecto(int codEncargadoObra,string nombreEncargadoObra, string celularEncargadoObra, string correoEncargadoObra)
         {
             string consulta = "update tb_encargado_obraproyectos set " +
                               " tb_encargado_obraproyectos.nombre = '" + nombreEncargadoObra + "', " +
                               " tb_encargado_obraproyectos.celular = '" + celularEncargadoObra + "', " +
                               " tb_encargado_obraproyectos.email = '" + correoEncargadoObra + "' " +
                               " where  " +
                               " tb_encargado_obraproyectos.codigo = " + codEncargadoObra;
             return conexion.ejecutarMySql(consulta);
         }

         internal bool insertarEncargadoObraProyecto(int codEdificio,string nombreEncargadoObra, string celularEncargadoObra, string correoEncargadoObra, string observacion)
         {
             string consulta = "insert into tb_encargado_obraproyectos( "+
                               " tb_encargado_obraproyectos.nombre, "+
                               " tb_encargado_obraproyectos.celular, "+
                               " tb_encargado_obraproyectos.email, "+
                               " tb_encargado_obraproyectos.observacion) "+
                               " values( "+
                               " '"+nombreEncargadoObra+"', "+
                               " '"+celularEncargadoObra+"', "+
                               " '"+correoEncargadoObra+"', "+
                               " '"+observacion+"')";
             bool bandera = conexion.ejecutarMySql(consulta);
             string consulta2 = "select max(tb_encargado_obraproyectos.codigo) "+ 
                                " from tb_encargado_obraproyectos";
             DataSet tupla = conexion.consultaMySql(consulta2);
             if (tupla.Tables[0].Rows.Count > 0)
             {
                 int codEncagadoObraProyecto = Convert.ToInt32(tupla.Tables[0].Rows[0][0].ToString());
                 string consulta3 = "update tb_proyecto set tb_proyecto.codencargadoproyecto = " + codEncagadoObraProyecto +
                                     " where " +
                                     " tb_proyecto.codigo = " + codEdificio;
                 bandera = conexion.ejecutarMySql(consulta3);
             }
             else
                 bandera = false;
         ...
         internal DataSet get_rcc(string nombreProyecto)
         {
             string consulta = "select res.codigo, res.nombre "+ 
                               " from tb_proyecto pp, tb_equipo eq, tb_responsable res "+
                               " where "+
                               " pp.codigo = eq.cod_proyecto and "+
                               " eq.cod_rcc = res.codigo and "+
                               " pp.nombre = '"+nombreProyecto+"' "+ 
                               " group by res.nombre";
             return conexion.consultaMySql(consulta);
         }

         internal DataSet get_fiscal(string nombreProyecto)
         {
             string consulta = "select res.codigo, res.nombre "+ 
                               " from tb_proyecto pp, tb_equipo eq, tb_responsable res "+
                               " where "+
                               " pp.codigo = eq.cod_proyecto and "+
                               " eq.codresponsable = res.codigo and "+
                               " pp.nombre = '"+nombreProyecto+"' "+
                               " group by res.nombre";
             return conexion.consultaMySql(consulta);
         }
