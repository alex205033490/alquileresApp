...
namespace jycboliviaASP.net.Datos
{
    public class DA_AgendaProyecto
    {
        ...
        //---------------------------------------------------
        public DataSet getAgendaporCodigoResponsable(int codigoResponsableAsignado)
        {
            string consulta = "select " +
                               " ane.codigo, " +
                               " DATE_FORMAT(ane.fechaasignacion,'%d/%m/%Y') as 'Fecha_Asignacion', " +
                               " ane.horaasignacion as 'HoraAsignacion', " +
                               " ane.edificio, " +
                               " ane.detalle as 'Objetivo_de_la_Agenda_Proyecto', " +                               
                               " ane.estado, " +                               
                               " DATE_FORMAT(ane.fechaexpiracion,'%d/%m/%Y') as 'Fecha_Expiracion', " +
                               " ane.horaexpiracion " +                               
                               " FROM " +
                               " tb_agenda_proyecto ane " +                                                              
                               " where  " +
                               " ane.estado = 'Abierto' and "+
                               " ane.codpersonalasignado = " + codigoResponsableAsignado;
            return cnx.consultaMySql(consulta);
        }

        public DataSet getAgendaporCodigoAgendaProyecto(int codigoAgenda)
        {
            string consulta = "select " +
                               " ane.codigo, " +
                               " DATE_FORMAT(ane.fechaasignacion,'%d/%m/%Y') as 'Fecha_Asignacion', " +
        ...
        internal bool cerrarAgendaProyecto(int codAgendaProyecto, string detalleCierre, int codUser, float costotransporte)
        {
            string consulta = "update tb_agenda_proyecto set "+
                               " tb_agenda_proyecto.estado = 'Cerrado', "+
                               " tb_agenda_proyecto.observacioncierre = '"+detalleCierre+"', "+
                               " tb_agenda_proyecto.fechacierre = current_date(), "+
                               " tb_agenda_proyecto.horacierre = current_time(), "+
                               " tb_agenda_proyecto.codrespcierre = "+codUser+" , "+
                               " tb_agenda_proyecto.costotransporte = '" + costotransporte.ToString().Replace(',','.') + "'"+
                               " where "+
                               " tb_agenda_proyecto.codigo =  "+codAgendaProyecto;
            return cnx.ejecutarMySql(consulta);
        }
    }
}