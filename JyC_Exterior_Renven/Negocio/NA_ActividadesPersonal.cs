...
namespace jycboliviaASP.net.Negocio
{
    public class NA_ActividadesPersonal
    {
        ...
        public bool updateCierreActividadPersonal(int codactividad, string detalleCierre, int codUserCierre, float costoPasaje)
        {
            return Dactividad.updateCierreActividadPersonal(codactividad, detalleCierre, codUserCierre, costoPasaje);
        }
        ...
        public bool updateCierreTareaTecnico(int codActividad, string detalleCierre, int codUserCierre, float costoPasaje)
        {
            return Dactividad.updateCierreActividadPersonal(codActividad, detalleCierre, codUserCierre, costoPasaje);
        }


        public DataSet mostrarActividades(bool estado, int codUsuarioAsignado)
        {
            string consulta = " select "+
                                " t.codigo, "+
                                " t.detalle as 'Actividad', "+  
                                " date_format(da.fechaasig,'%d/%m/%y') as 'FechaAsignacion', "+ 
                                " da.horaasig as 'HoraAsignacion', "+ 
                                " date_format(da.fechaejecucion,'%d/%m/%y') as 'FechaEjecucion' , "+
                                " da.horaejecucion as 'HoraEjecucion', "+
                                ...
                                " where "+
                                " t.estado = "+estado+"  and "+
                                " t.coduserultimoasignado = "+ codUsuarioAsignado+
                                " order by t.fecha desc";
            return Dactividad.getDatos(consulta);
        }

        internal DataSet get_ActividadporCodigo(int codActividadPersonal)
        {
            string consulta = " select " +
                                " t.codigo, " +
                                " t.detalle as 'Actividad', " +
