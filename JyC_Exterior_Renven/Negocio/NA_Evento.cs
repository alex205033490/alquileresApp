...
namespace jycboliviaASP.net.Negocio
{
    public class NA_Evento
    {
        ...
        public DataSet getInformeDeAtencionCallCenter(string fecha1, string fecha2, string baseDeDatos)
        {
            string consulta = "select "+
                                   " even.codigo as 'Ticket', "+
       ...
       private string get_PromedioAtencionEmergenciaBaseDatos_AscensorParado_NOCHE(int mes, int anio, string DB)
       {
        ...
        ///------------------------nuevo ----------------------
        ///
        public DataSet get_EmergenciaTecnico(int codTecnico) { 
            DataSet emergencias = Devento.getEmergenciaTecnico(codTecnico);
            return emergencias;        
        }

        internal DataSet getDatosEmergenciaCallcenter(int codigoDetalleEmergencia)
        {
            return Devento.getDatosEmergenciaCallcenter(codigoDetalleEmergencia);
        }
        ...
        {
            return Devento.guardarDatosEmergenciaCallcenter(   fechallegada ,  horallegada ,                                              
                                                         i_fusiblecontactos , i_botoneradepisoencorte , i_limites,
                                                         i_reguladordevelocidad , i_frenobalataselectroiman ,
                                                         i_motordetraccion , i_poleas , i_filtraciondeaguaensalademaquinas ,
                                                         i_accesoirregularasalademaquinas , i_corteensenalizadoropulsadordepiso ,
                                                         i_ruidooajusteenpuertaspisocabina , i_iluminaciondecabina , i_operadordepuertas ,
                                                         i_motordeoperador , i_ventiladordecabina , i_cerrojo ,
                                                         i_sensordecabinabarrerafotocelula , i_filtraciondeaguaenhuecoyfoso ,
                                                         i_bajasocortedetension , i_sensores , i_malusoporusuario ,
                                                         i_iluminacionirregularensalademaquinasyfoso , i_otros ,
                                                         cambiorepuesto , arreglo ,  recepcion , receptor_ci ,
                                                         receptor_cargo , observacioncierre ,
                                                         cod_Tarea_Tecnico, codresgra, siningresoedificio, estadoboleta, ascensor, costoPasaje);
           
        }

        

        internal int getCodigoEmergenciaCallcenter(int cod_detalleEventoCallcenter)
        {
            DataSet tuplas = Devento.get_CodigoEmergenciaCallcenter(cod_detalleEventoCallcenter);
            if (tuplas.Tables[0].Rows.Count > 0)
            {
                return Convert.ToInt32(tuplas.Tables[0].Rows[0][0].ToString());
            }
            else
                return -1;            
        }

        public DataSet get_DatosEmergenciaCallcenter(int cod_Tarea_TecnicoBoleta) {
           // int CodigoBoletaEmergencia = getCodigoEmergenciaCallcenter(cod_detalleEventoCallcenter);
            DataSet tuplas = Devento.get_DatosEmergenciaCallcenter(cod_Tarea_TecnicoBoleta);
            return tuplas;
        }


        internal int get_cod_detalleEventoCallcenter(int cod_Tarea_Tecnico)
        {
            DataSet tuplas = Devento.get_cod_detalleEventoCallcenter(cod_Tarea_Tecnico);
            if (tuplas.Tables[0].Rows.Count > 0)
            {
